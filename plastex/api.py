...
def get_email_list():
    ...
    email_list = frappe.db.sql(
        """select email_id from `tabEmail Account` """, as_dict=1
    )
...
def get_print_format(doctype):
    print_list = []
    print_format = frappe.db.sql(
        """select name from `tabPrint Format` where doc_type = %s""", doctype, as_dict=1
    )
...
def get_email_template(value):
    template = frappe.get_list(
        "Email Template", filters={"name": value}, fields=["response", "subject"]
    )
    return template
...
def send():
    ...
    email_queue = frappe.get_list(
        "Email Queue",
        filters={"reference_doctype": "Purchase Order", "status": "Not Sent"},
        fields=["name", "communication"],
    )
    for email in email_queue:
        comm = frappe.get_value(
            "Communication", email["communication"], "sent_via_send_mail"
        )
